...
func loadV1(r *gin.Engine, svcCtx *svc.ServerCtx) {
	...
	collections := apiV1.Group("/collections")
	{
		...
		collections.POST("/createNft", middleware.AuthMiddleWare(svcCtx.KvStore), v1.CreateNft(svcCtx)) // 获取NFT集合排名信息
	}
	...
	portfolio := apiV1.Group("/portfolio").Use(middleware.AuthMiddleWare(svcCtx.KvStore))
	{
	...
	//uploadFile.Use(middleware.AuthMiddleWare(svcCtx.KvStore))
	{
		...
		uploadFile.POST("/getFile", v1.GetFile(svcCtx))
